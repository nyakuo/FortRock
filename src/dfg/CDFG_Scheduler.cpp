  ...
  for (auto & state_dfg : dfg) {
    ...
    for (auto & elem : tmp_dfg) {
      if (elem->get_operator()->get_type()
        ...
        std::cout << "br: "
                  << this->_get_last_step(tmp_dfg) << std::endl;
...
(const std::list<std::shared_ptr<CDFG_Element> > & list,
 const std::shared_ptr<CDFG_Element> & target_elem) {
  ...
  for (auto i=0; i < target_elem->get_num_input();
    ...
    // 入力が即値の場合は考慮しない
    if (target_elem->get_input_at(i)->get_type()
    ...
    for (auto & elem : cp_list) {
      ...
      // 同一基本ブロック内で定義された変数が存在する場合
      // 最小ステップを更新
      if (elem->get_output_at(0)
          == target_elem->get_input_at(i))
        min_step
          = std::max(min_step,
                     elem->get_step()
  ...
  for (auto & start_elem : target_elem) {
    ...
    for (auto & another_elem : target_elem) {
      ...
      if (start_elem->get_step() + latency >
        ...
        // 実行不可能
        if (another_elem->get_step() - start_elem->get_step()
            < latency) {
          can_use = false;
          break;
        }
      ...
      // 新たなタイミングで実行可能な場合
      if (can_use == true) // (target_elemはソート済み)
        return start_elem->get_step() + latency + 1;

    } // for : another_elem
  } // for : target_elem

  return this->_get_last_step(target_elem);
} // _min_step_operator
